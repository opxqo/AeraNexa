...
3. ~~`/demo/*` 纯前端 Mock 演示模式~~：已于 2026-09-25 删除。
...
`src/components/admin-editor.tsx` 是 `"use client"` 客户端组件，而既有用例断言的是
**服务端返回的 HTML 字符串**。如果它在浏览器里 hydration 报错或客户端崩溃，
这些断言照样全绿——这是一个真实存在的盲区。

因此用 `agent-browser` 做了一次冒烟：管理员身份登录 → 打开 `/admin/traffic`：

- **页面错误 0、控制台错误/警告 0**（客户端渲染干净）；
- 两张汇总卡片分别渲染：日粒度 38.00 GB（上行 13 + 下行 25）、小时粒度 3.00 GB；
- 表格 3 行，节点名与 ID、粒度徽章（日 / 小时）、字节格式化均正确；
- **时间按东八区正确回显**：上报的 `2026-09-19` 显示为 `2026/09/19 00:00`，
  `2026-09-19 10:00:00` 显示为 `2026/09/19 10:00`——印证了「存 UTC、按东八区展示」的口径；
- 在搜索框输入节点名并点「筛选」：表格 3 行 → 2 行，**汇总卡片同步变为
  「3.00 GB / 5.00 GB / 1 个节点」**——「汇总跟随搜索条件」在真实交互下成立。

冒烟数据（临时用户、节点与流量记录）已清理，收尾核对 `users` 1、`nodes` 0、两张流量表 0。

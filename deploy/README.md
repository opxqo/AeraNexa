...
| 从旧面板迁移 | 搬家：在旧面板「数据迁移」页生成迁移码粘贴进来，直接在线拉取数据和密钥；也可以填备份文件路径 |
...
2. 旧面板后台「数据迁移」→「生成迁移码」。迁移码 30 分钟内有效，**只能使用一次**；传输中断就重新生成一个。
3. 新服务器执行一键安装脚本，选「从旧面板迁移」，把迁移码粘贴进去。脚本会装好环境、从旧面板拉取全部数据和加密主密钥、写入 `.env.local`、启动服务。
